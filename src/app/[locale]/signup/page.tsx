...
import { ArrowRight, Mail, Lock, User, Music4 } from "lucide-react";
        ...
        <div className="bg-white/80 dark:bg-zinc-900/60 backdrop-blur-xl border border-zinc-200 dark:border-white/10 rounded-3xl p-8 shadow-2xl dark:shadow-black/50">
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="relative flex items-center py-2">
              ...
              <span className="flex-shrink-0 mx-4 text-zinc-400 text-sm">{t("orSignupEmail")}</span>
              <div className="flex-grow border-t border-zinc-200 dark:border-white/10"></div>
            </div>

            <div className="space-y-1.5">
              <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300 ml-1">{t("nameLabel")} <span className="text-zinc-400 font-normal">{t("nameOptional")}</span></label>
              <div className="relative">
                <User className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-zinc-400" />
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} autoComplete="name" placeholder="John Doe"
            ...
            <button type="submit" disabled={submitting}
              ...
              {submitting ? (
                ...
                <>{t("signupBtn")} <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" /></>
              )}
            </button>
          </form>
