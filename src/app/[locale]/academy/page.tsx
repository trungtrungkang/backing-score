    ...
    <main className="min-h-screen bg-zinc-50 dark:bg-[#0A0A0A] text-zinc-900 dark:text-white pb-24 font-sans">
      ...
      <section className="max-w-7xl mx-auto px-6 lg:px-12 pt-16">
        <div className="flex flex-col gap-3 mb-10 items-start">
           <h2 className="text-2xl font-bold tracking-tight">{t("featuredCourses")}</h2>
           <span className="text-sm font-semibold text-blue-500 bg-blue-50 dark:bg-blue-500/10 px-3 py-1 rounded-full w-fit">
             {t("coursesAvailable", { count: publishedCourses.length })}
           </span>
        </div>
        ...
        {publishedCourses.length === 0 ? (
          <div className="w-full py-32 flex flex-col items-center justify-center border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-3xl">
             ...
             <h3 className="text-lg font-bold text-zinc-500">{t("noCoursesTitle")}</h3>
             <p className="text-sm text-zinc-400">{t("noCoursesSubtitle")}</p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="p-5 sm:p-6 flex flex-col flex-1">
                  <div className="flex items-center gap-2 mb-3 flex-wrap">
                     <span className="text-[10px] font-bold uppercase tracking-widest text-indigo-500 bg-indigo-50 dark:bg-indigo-500/10 px-2 py-0.5 rounded whitespace-nowrap">
                       {course.category || t("musicTheory")}
                     </span>
                     <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-500 bg-emerald-50 dark:bg-emerald-500/10 px-2 py-0.5 rounded whitespace-nowrap">
                       {course.difficulty || t("beginner")}
                     </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between border-t border-zinc-100 dark:border-zinc-800 pt-4 mt-2">
                    <div className="flex flex-col">
                      <span className="text-[10px] uppercase font-bold text-zinc-400 tracking-wider">{t("tuition")}</span>
                      <span className="text-lg font-black text-zinc-900 dark:text-white">
                        {course.priceCents > 0 ? `$${(course.priceCents / 100).toFixed(2)}` : t("free")}
                      </span>
                    </div>
