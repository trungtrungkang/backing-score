  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950">
      ...
      <div className="relative overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(200,168,86,0.15),transparent_50%)]" />
        <div className="relative max-w-5xl mx-auto px-4 pt-20 pb-12 text-center">
          ...
          {/* Billing Toggle */}
          <div className="inline-flex items-center gap-3 bg-zinc-800/50 rounded-full p-1 border border-zinc-700/50">
            <button
      ...
      <div className="max-w-5xl mx-auto px-4 pb-20">
        <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-8 flex flex-col">
            <h3 className="text-lg font-bold text-zinc-300 mb-1">{t("freeTier")}</h3>
            <p className="text-sm text-zinc-500 mb-6">{t("freeDesc")}</p>
            <div className="mb-8">
              <span className="text-4xl font-bold text-white">$0</span>
              <span className="text-zinc-500 ml-1">/ {t("forever")}</span>
            </div>
            <ul className="space-y-3 mb-8 flex-1">
              {freeFeatures.map((f, i) => (
                <li key={i} className="flex items-start gap-3">
                  <f.icon className="w-5 h-5 text-zinc-500 mt-0.5 shrink-0" />
            ...
            {!user ? (
              <Link
                href="/en/signup"
                className="w-full py-3 rounded-xl border border-zinc-700 text-center text-sm font-bold text-zinc-300 hover:bg-zinc-800 transition-colors"
              >
                {t("getStarted")}
              </Link>
            ) : (
              <div className="w-full py-3 rounded-xl border border-zinc-700/50 text-center text-sm text-zinc-500">
                {t("currentPlan")}
              </div>
            )}
          </div>
          ...
          {/* Premium Tier */}
          <div className="rounded-2xl border-2 border-[#C8A856]/40 bg-gradient-to-b from-[#C8A856]/5 to-zinc-900/50 p-8 flex flex-col relative">
            ...
            <p className="text-sm text-zinc-400 mb-6">{t("premiumDesc")}</p>
            <div className="mb-8">
            ...
            {isPremium ? (
              <div className="w-full py-3 rounded-xl bg-[#C8A856]/10 border border-[#C8A856]/30 text-center text-sm font-bold text-[#C8A856]">
                ...
                {subscriptionStatus === "cancelled" && (
                  <span className="block text-xs text-zinc-400 mt-1">{t("cancelledNote")}</span>
                )}
              </div>
            ) : (
              <button
