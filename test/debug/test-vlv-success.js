...
verovio.module.onRuntimeInitialized = () => {
    ...
    // MOVED: SVG Visual Accidental Fix natively applied to the VISUAL DOC
    const svgNotes = baseXmlDoc.getElementsByTagName('note');
    for (let i = 0; i < svgNotes.length; i++) {
        const noteNode = svgNotes[i];
        ...
        if (pitch && ExistingAccidental.length === 0) {
            ...
            const accNode = baseXmlDoc.createElement('accidental');
    ...
    // TIE FIX (Applied exclusively to MIDI rendering)
    ...
    console.log("Verovio load success:", success);
