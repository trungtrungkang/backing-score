...
| **Stripe = Payment layer** | Tách biệt Stripe Connect (marketplace) khỏi LemonSqueezy (subscription) |
...
│                                                                         │
│  ┌─────────────────┐ ┌─────────────────┐ ┌─────────────────┐           │
│  │ Appwrite Cloud   │ │ LiveKit Cloud    │ │ Stripe Connect   │          │
│  │ DB, Auth, Func,  │ │ WebRTC, DataCh,  │ │ Payment, Payout  │          │
│  │ Realtime, Storage│ │ Recording        │ │ Subscriptions    │          │
│  └─────────────────┘ └─────────────────┘ └─────────────────┘           │
│                                                                         │
│  ┌─────────────────┐ ┌─────────────────┐                               │
│  │ Vercel Edge      │ │ LemonSqueezy     │                              │
│  │ CDN, SSR, Func   │ │ (existing B2C)   │                              │
│  └─────────────────┘ └─────────────────┘                               │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| `Stripe Connect` | Marketplace payments, teacher payout | Payment |
| `@stripe/stripe-js` | Stripe Elements (card input) | Frontend |
| `stripe` | Server-side Stripe SDK | Backend |
...
├── submissions.ts       # Submit, update status, query by student/assignment
...
### 5.3 Stripe Service — `src/lib/stripe/`

```
src/lib/stripe/
├── connect.ts           # createConnectAccount, getLoginLink
├── checkout.ts          # createPaymentIntent per booking
├── payout.ts            # calculatePayout, createTransfer
└── webhook.ts           # handleEvent: payment_intent.succeeded, etc.
```
...
| `/api/stripe/connect` | POST | Tạo Stripe Connect onboarding link |
| `/api/stripe/checkout` | POST | Tạo PaymentIntent cho booking |
| `/api/stripe/webhook` | POST | Xử lý Stripe webhook events |
| `/api/classroom/join` | POST | Student join classroom via code |
| `/api/booking/create` | POST | Tạo booking + PaymentIntent |
...
│                   Payment Flows                       │
├──────────────────────┬───────────────────────────────┤
│ B2C Subscriptions    │ Marketplace Per-Session       │
│ (existing)           │ (new)                         │
│                      │                               │
│ LemonSqueezy         │ Stripe Connect                │
│ Student Premium      │ Student → Stripe → Payout     │
│ $4.99/tháng          │ $10-80/session                │
│ Teacher Pro           │ B&S: 15% commission          │
│ $9.99/tháng          │ Teacher: 85%                  │
└──────────────────────┴───────────────────────────────┘
```
...
sequenceDiagram
    ...
    B->>ST: createPaymentIntent($30, transfer_data: teacher)
    ST-->>B: paymentIntent { client_secret }
    B-->>S: { bookingId, clientSecret }
    S->>ST: confirmPayment(clientSecret, card)
    ST-->>S: Payment succeeded
    ...
    B->>ST: createTransfer($25.50 to teacher)
    ST->>T: Funds available in Stripe balance
```
...
```mermaid
flowchart TD
    A[Teacher bấm Become a Teacher] --> B[Server tạo Stripe Connect account]
    B --> C[Redirect → Stripe onboarding form]
    C --> D[Teacher nhập bank info, ID]
    D --> E[Stripe verify → account active]
    E --> F[stripeConnectId saved to teacher_profiles]
    F --> G[Teacher có thể nhận thanh toán]
```
...
| Stripe payments | ~200/tháng | ❌ | Well under limits |
...
| Payment fraud | Stripe handles, webhook signature verification |
...
| Stripe Connect payout delay | Thấp | Trung bình | Clear communication, 7-day hold |
...
gantt
    ...
    Stripe Connect + checkout  :2026-09, 1.5w
